string MD5(string str)
{
    ...
    MD5Transform(a, b , c, d, last);
    string result;
    StringConcatenate(result, IntegerToString(a) , IntegerToString(b) , IntegerToString(c) ,  IntegerToString(d));
    return result;
}
...
string IntegerToString(int integer_number)
{
   ...
   for (int i = 0; i < 4; i++)
   {
       ...
       StringConcatenate(hex_string, hex_string, hex_item);
   }
   return(hex_string);
}
...
string Dec2Hex(int num)
{
    ...
    for (int i =0; i < 2; i++)
    {
        modnum = num % 16;
        num = (num - modnum) / 16;
        StringConcatenate(hex, IntToHexString(modnum), hex);
    }
    return (hex);
}
